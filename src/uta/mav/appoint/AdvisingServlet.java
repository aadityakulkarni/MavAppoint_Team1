...
public class AdvisingServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			if (studentUser == null) {
				if (user != null && user.getRole() != null && user.getRole().equalsIgnoreCase("advisor")) {
					AdvisorUser advUser = dbm.getAdvisor(user.getEmail());
					int tempType = advUser.getDegType();
					...
					// user = new LoginUser();
					session.setAttribute("user", advUser);
					...
					for (int depIndex = 0; depIndex < tempDeps.size(); depIndex++) {
						if (user.getDepartments().contains(
								tempDeps.get(depIndex).getName())) {
							...
							for (int majorIndex = 0; majorIndex < tempDeps
									.get(depIndex).getMajors().size(); majorIndex++)
								if (user.getMajors().contains(
										tempDeps.get(depIndex).getMajors()
												.get(majorIndex))) {
									// System.out.println("Found major "+tempDeps.get(depIndex).getMajors().get(majorIndex));
									tempMajors.add(tempDeps.get(depIndex)
											.getMajors().get(majorIndex));
								}
				...
				} else if (user != null && user.getRole() != null && user.getRole().equalsIgnoreCase("admin")) {
					AdminUser adminUser = dbm.getAdmin(user.getEmail());
					int tempType = adminUser.getDegType();
					degreeType = new ArrayList<>();
					if (tempType >= 4) {
						tempType -= 4;
						degreeType.add("Doctorate");
			...
			advisors = dbm
					.getAdvisorsOfDepartment(departments.get(0).getName());
			if (advisors.size() != 0) {
				session.setAttribute("advisors", advisors);
			}
			ArrayList<TimeSlotComponent> schedules = dbm
					.getAdvisorSchedules(advisors);
			if (schedules.size() != 0) {
		...
		// System.out.println("majors selected --------------- deps" +
		// major.get(0));
		request.setAttribute("includeHeader", header);
		request.getRequestDispatcher("/WEB-INF/jsp/views/advising.jsp")
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			Integer departmentIndex = Integer.valueOf(request
					.getParameter("drp_department"));
			...
			{
				Integer majorIndex = Integer.valueOf(request
						.getParameter("drp_major"));
			...
			Integer letterIndex = Integer.valueOf(request
					.getParameter("drp_lastName"));
			...
			Integer degreeIndex = Integer.valueOf(request
					.getParameter("drp_degreeType"));
			...
			if (advisor != null && !advisor.equals("all")) {
				...
				for (int i = 0; i < advisors.size(); i++) {
					...
					if (advisors.get(i).advisesStudent(tempDep, tempMajor,
							selectedLetter, degreeValue)) {
			...
			/*
			 * ArrayList<Object> appointments = dbm.getAppointments(user); if
			 * (appointments.size() != 0){ session.setAttribute("appointments",
			 * appointments); }
			 */
		} catch (Exception e) {
